    ...
    <div className="max-w-7xl mx-auto px-6 py-12 space-y-16">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
        ...
        <div className="space-y-4">
          <div className="bg-bazaario-card border border-bazaario-border p-6 rounded-2xl flex items-center justify-center aspect-square relative shadow-[0_0_30px_rgba(20,184,166,0.05)]">
            <img
            ...
            />
            <button className="absolute top-4 right-4 p-3 bg-[#0A0A0C]/80 hover:bg-[#0A0A0C] text-gray-400 hover:text-red-500 rounded-full border border-bazaario-border transition-all">
              <Heart size={18} />
            </button>
          </div>
          ...
          <div className="flex gap-3 justify-center">
            {productImages.map((img, idx) => (
              <button
                ...
                className={`w-16 h-16 bg-bazaario-card p-1.5 rounded-lg border transition-all flex items-center justify-center overflow-hidden ${
                  selectedImgIndex === idx ? 'border-bazaario-primary scale-105' : 'border-bazaario-border opacity-60 hover:opacity-100'
                }`}
              >
                <img 
                  src={img} 
        ...
        <div className="space-y-6">
          ...
          {/* Actions */}
          {product.stockQty > 0 && (
            <div className="space-y-4 pt-4 border-t border-bazaario-border">
              {/* Stepper + Buttons */}
              <div className="flex flex-col sm:flex-row gap-4 items-center">
                {/* Quantity select */}
                <div className="flex items-center bg-[#0A0A0C] border border-bazaario-border rounded-full px-4 py-2 flex-shrink-0 w-32 justify-between">
                  <button
                ...
                <button
                  ...
                  className="w-full bg-bazaario-primary hover:bg-bazaario-primaryHover text-black font-extrabold py-3.5 rounded-full transition-all text-xs uppercase tracking-wider shadow-[0_0_15px_rgba(20,184,166,0.25)] flex items-center justify-center gap-2"
                >
                  Add to Cart
                </button>
              </div>
              ...
              <button
                ...
                className="w-full border border-bazaario-primary text-bazaario-primary hover:bg-bazaario-primary hover:text-black font-extrabold py-3.5 rounded-full transition-all text-xs uppercase tracking-wider flex items-center justify-center gap-2"
              >
                Buy Now
              </button>
            </div>
      ...
      <div className="space-y-6 pt-6 border-t border-bazaario-border">
        <div className="flex gap-8 border-b border-bazaario-border pb-px text-sm font-bold">
          <button
          ...
          {activeTab === 'reviews' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
              ...
              <div className="lg:col-span-1 space-y-6">
                ...
                {token ? (
                  <form onSubmit={handleReviewSubmit} className="bg-bazaario-card border border-bazaario-border p-6 rounded-2xl space-y-4">
                    ...
                    <button
                      ...
                      className="w-full bg-bazaario-primary hover:bg-bazaario-primaryHover text-black text-xs font-bold py-2.5 rounded-full transition-all uppercase tracking-wider"
                    >
                      {submittingReview ? "Submitting..." : "Submit Review"}
                    </button>
                  </form>
                ) : (
                  <div className="bg-bazaario-card border border-bazaario-border p-6 rounded-2xl text-center space-y-3">
              ...
              <div className="lg:col-span-2 space-y-4">
                ...
                {reviews.length === 0 ? (
                  <div className="bg-bazaario-card border border-bazaario-border p-8 rounded-2xl text-center text-gray-500 text-xs">
                    No reviews yet. Be the first to review this product!
                  </div>
                ) : (
                  <div className="space-y-4">
                    {reviews.map((rev) => (
                      <div key={rev.id} className="bg-bazaario-card border border-bazaario-border p-5 rounded-2xl space-y-3">
